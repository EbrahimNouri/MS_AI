...
SCORE_TABLE = {1: 40, 2: 100, 3: 300, 4: 1200}  # single, double, triple, tetris (x level+1)
...
class Tetris:
    ...
    def clear_lines(self):
        full_rows = [y for y in range(ROWS) if all(self.grid[y][x] is not None for x in range(COLS))]
        if not full_rows:
            return
        for y in full_rows:
            del self.grid[y]
    ...
    def draw_cell(self, surface, px, py, color, alpha=255):
        ...
        if alpha < 255:
            ...
            pygame.draw.rect(s, (*color, min(255, alpha + 60)), s.get_rect(), width=2, border_radius=4)
    ...
    def draw_board(self):
        ...
        for x in range(COLS + 1):
            xpos = BOARD_X + x * CELL
            pygame.draw.line(self.screen, GRID_LINE, (xpos, BOARD_Y), (xpos, BOARD_Y + BOARD_H))
        for y in range(ROWS + 1):
            ypos = BOARD_Y + y * CELL
            pygame.draw.line(self.screen, GRID_LINE, (BOARD_X, ypos), (BOARD_X + BOARD_W, ypos))
        ...
        for y in range(ROWS):
            for x in range(COLS):
                color = self.grid[y][x]
                if color:
                    self.draw_cell(self.screen, BOARD_X + x * CELL, BOARD_Y + y * CELL, color)
        ...
        if not self.game_over and not self.paused:
            ...
            for x, y in self.current.cells(y=ghost_y):
                if y >= 0:
                    self.draw_cell(self.screen, BOARD_X + x * CELL, BOARD_Y + y * CELL,
                                    self.current.color, alpha=GHOST_ALPHA)
            # Current piece
            for x, y in self.current.cells():
                if y >= 0:
                    self.draw_cell(self.screen, BOARD_X + x * CELL, BOARD_Y + y * CELL, self.current.color)
    ...
    def draw(self):
        ...
        if self.paused and not self.game_over:
            ...
            self.draw_center_message("GAME OVER", f"Score: {self.score}   |   Press R to restart")
